...
export default function HeroSection() { const [mounted, setMounted] = useState(false);

useEffect(() => { setMounted(true); }, []);

return ( <section className="relative overflow-hidden min-h-screen flex flex-col items-center justify-center text-center py-24 px-6 bg-gradient-to-b from-black via-gray-900 to-black"> {/* Background Stars */} <div className="absolute inset-0 z-0"> {stars.map((_, index) => ( <motion.div key={index} className="absolute bg-white rounded-full opacity-50" style={{ width: ${Math.random() * 3 + 2}px, height: ${Math.random() * 3 + 2}px, top: ${Math.random() * 100}%, left: ${Math.random() * 100}%, }} animate={{ y: [0, 20, 0] }} transition={{ duration: Math.random() * 5 + 5, repeat: Infinity, repeatType: "reverse", ease: "easeInOut", }} /> ))} </div>
    ...
    >
      <div className="relative w-40 h-40">
        <Image
          ...
          layout="fill"
          objectFit="contain"
          className="drop-shadow-lg rounded-full glow-animation"
        />
        <div className="absolute top-full left-0 w-full h-full opacity-20 transform scale-y-[-1] blur-md">
          <Image
            src="/images/logo.png"
            alt="Reflection"
            layout="fill"
